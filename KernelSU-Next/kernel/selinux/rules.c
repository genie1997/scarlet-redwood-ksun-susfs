...
/*
 * Wire format (KernelSU-Next >= v3.3.0 ksud):
 *   repeated: [struct sepol_data hdr][arg]*argc
 *   arg := [u32 len][len bytes][NUL];  len == 0 means ALL.
 * Argument strings are inline in the payload, NOT userspace pointers as in the
 * v3.1.0 layout this driver originally shipped.
 */
struct sepol_data {
    u32 cmd;
    u32 subcmd;
...
out_unlock:
    ...
    // selectively here, so we just reset it every time.
    reset_avc_cache();
    mutex_unlock(&ksu_rules);
out_free:
    kvfree(payload);
